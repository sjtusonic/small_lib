...
if $DEBUG_inc [puts "DEBUG: begin [info script]"]
...
#puts "ENTER show_var";
#puts -nonewline "CALLING show_var:";
	set cntr [info level] ;
	while {1} {
#puts "info frame $cntr =[info frame $cntr]";
		set iff [info frame $cntr];

		set callerFileTail [lindex [split [dict get $iff file] "/"] end];
		set callerLineNum                 [dict get $iff line];
		set callerProc "NA";
		set callerCmd [dict get $iff cmd];
#puts "cmd=$callerCmd ";
		if {![regexp {^show_var} $callerCmd ]} {
#puts "ASSERT FAIL! cmd=$callerCmd ";
			incr cntr
		} else {
			break;
		}
	}
	if {[dict exist $iff proc]} {set callerProc [dict get $iff proc];}
	foreach varName $varNameList {
		upvar 1 $varName var;
		if {[info exists var]} {
			puts "show_var: $varName=$var @$callerFileTail:$callerLineNum proc=$callerProc";
		}
	}
#puts "RET show_var";
}
...
proc show_array {arrayName} {
	puts "call update $arrayName";

	upvar #0 $arrayName cfg;

	puts "=================================";
	puts "show_array $arrayName:";
	foreach i [lsort -dic [array names cfg]] {
		puts "$i=>$cfg($i)";
	}
	puts "=================================";
}
proc floor {x} {
	if {$x >=0} {
		for {set i 0} {$i<999999999 } {incr i} {
			if {$i>$x} {
				return [expr $i-1];
			}
		}
